...
for i = 1:size(A, 1)
    for j = 1:size(A, 2)
        dot_product = dot(squeeze(A(i, j, :)), b);
        Psi(i, j) = quaternion( ...
...
title( ...
  ['Imaginary Part of Spherical Zonal Monogenics $k=2$, $K_{2}(x,\eta_{2})$, ' ...
   'the red point $\eta_{2}=(-0.33217  -0.75213   0.56916)$'], ...
  'Interpreter','latex', 'FontSize',20);

hold on
scatter3(b(1), b(2), b(3), 80, 'r', 'filled');

% ---- label eta_1 on the sphere ----
text(b(1), b(2), b(3), ' $\eta_{2}$', ...
     'Interpreter','latex', ...
     'FontSize',20, ...
...
title( ...
  ['Scalar Par;xxt of Spherical Zonal Monogenics $k=2$, $K_{2}(x,\eta_{2})$, ' ...
   'the red pointx $\eta_{2}=(-0.33217  -0.75213   0.56916)$'], ...
  'Interpreter','latex', 'FontSize',20);
